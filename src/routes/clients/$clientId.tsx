...
function ClientDetailPage() {
  ...
  const [name, setName] = useState(detail.client.name), [email, setEmail] = useState(detail.client.email || ""), [phone, setPhone] = useState(detail.client.phone || ""), [address, setAddress] = useState(detail.client.address || "");
  const save = async (e: React.FormEvent) => { e.preventDefault(); setError(null); if (!name.trim()) return setError("Client name is required"); if (email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) return setError("Enter a valid email address"); try { setSaving(true); await updateClient({ data: { clientId: detail.client.id, name, email, phone, address } }); setEditing(false); await navigate({ to: ".", reloadDocument: true }); } catch (err) { setError(err instanceof Error ? err.message : "Unable to save client"); } finally { setSaving(false); } };
