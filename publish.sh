...
# Load .env file to supply DATABASE_URL (and other vars) to the server process.
# Bun auto-loads .env but system env vars take precedence — sourcing here ensures
# the .env values win for the server's lifetime.
if [ -f .env ]; then
  set -a
  source .env
  set +a
